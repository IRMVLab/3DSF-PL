...
def process_one_frame(idx):
    ...
    #valid_px1 = valid_pc1.reshape(375,1241,3)
    #valid_px2 = valid_pc2.reshape(375,1241,3)
    truenas_path = osp.join(save_path, '{:06d}'.format(idx))
    os.makedirs(truenas_path, exist_ok=True)
    np.save(osp.join(truenas_path, 'final_mask1.npy'), final_mask)
    np.save(osp.join(truenas_path, 'final_mask2.npy'), final_mask)
