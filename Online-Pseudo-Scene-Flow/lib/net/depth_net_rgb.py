    ...
    def eval(self, batch, max_high=1.0):
        ...
        with torch.no_grad():
            ...
            for depth, calib_info, image, sample_id in zip(
                ...
                cloud = depth_to_pcl(calib_info, depth,image, max_high=max_high)
                cloud = filter_cloud(cloud, image, calib_info)
                #save_pcl(cloud, '../../dataset/data_odometry_color/sequences/02/PL_rgb/{:06d}'.format(sample_id))
                #cloud = cloud
                #save_pcl(cloud, 'points/nosparse_{}'.format(sample_id))
                #cloud = transform(cloud, calib_info, sparse_type='angular_min', start=2.0)
            ...
            point1 = points[:13:12]
            point2 = points[1:14:12]
            point3 = points[2:15:12]
            point4 = points[3:16:12]
            point5 = points[4:17:12]
            point6 = points[5:18:12]
            point7 = points[6:19:12]
            point8 = points[7:20:12]
            point9 = points[8:21:12]
            point10 = points[9:22:12]
            point11 = points[10:23:12]
            point12 = points[11:24:12]
            point5 = points[8:10]
            point6 = points[10:12]
            point7 = points[12:14]
            point8 = points[14:16]
            point9 = points[16:18]
            point10 = points[18:20]
            point11 = points[20:22]
            point12 = points[22:24]
            
            out17,out18 = aum(point9,sample_id)
            out19,out20 = aum(point10,sample_id)
            out21,out22 = aum(point11,sample_id)
            out23,out24 = aum(point12,sample_id)
            ...
            points1 = torch.stack((out1,out3,out5,out7), dim=0)
            points2 = torch.stack((out2,out4,out6,out8), dim=0)
...
def depth_to_pcl(calib, depth, image, max_high=1.):
    ...
    img = np.array(image)
    img = torch.as_tensor(torch.from_numpy(img), dtype=torch.float32)
    image = img.reshape(-1,3).cuda()
    
    cloud = torch.cat((cloud,image), dim=1)
    #point_cloud = pts_rect.detach().cpu()
...
def filter_cloud(velo_points, image, calib):
    ...
    '''idxxx = torch.randperm(len(velo_points))
    num = len(velo_points) // 2
    indxx1 = idxxx[:num]
    velo_points = velo_points[indxx1]'''
    pc_data = velo_points[:,:3].cpu().numpy()
    data = FR.FliterR(pc_data)
    data,index = data.fliter_radius(pc_data)
    velo_points = velo_points[index]
    #velo_points = torch.as_tensor(torch.from_numpy(data), dtype=torch.float32)
    #velo_points = velo_points.cuda()
...
'''def fliter_radius(pc_data):
    ...
    pc_data = np.delete(pc_data,-1,axis=1)
    ...
    velo_points = torch.from_numpy(cl_arr)
...
def aum(point,sample_id):
    pc1 = point[0]
    pc2 = point[1]
    #save_pcl(pc1, 'points/sparse_{}'.format(sample_id-1))
    #save_pcl(pc2, 'points/sparse_{}'.format(sample_id))
    len1 = len(pc1)
    len2 = len(pc2)  
    if len1 < len2:
        indx = len1
    ...
    pc1_med3 = torch.cat((pc1_med3,c1),1)
    pc2_med5 = torch.cat((pc2_med5,c2),1)
    indxx1 = idx1[:4096]
    indxx2 = idx2[:4096]
