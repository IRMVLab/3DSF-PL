...
class KittiRCNNDataset(KittiDataset):
    ...
    def get_label(self, idx):
        if idx < 10000:
            label_file = os.path.join(self.label_dir, '%010d.txt' % idx)
        else:
            label_file = os.path.join(self.aug_label_dir, '%010d.txt' % idx)
    ...
    def get_rpn_sample(self, index):
        ...
        if sample_id < 10000:
            ...
            calib2 = self.get_calib(sample_id+1)
            img_left2 = self.get_image((sample_id+1) % 10000, left_image=True)
            img_right2 = self.get_image((sample_id+1) % 10000, left_image=False)            
            # img_shape = self.get_image_shape(sample_id)
            W2, H2 = img_left2.size
            depth2 = self.get_depth(sample_id+1)

            # Pad depth to constant shape for batching
            top_pad2 = 384 - H2
            right_pad2 = 1248 - W2
            depth2 = np.pad(depth2, ((top_pad2, 0), (0, right_pad2)), 'constant', constant_values=0)

        sample_info = {'sample_id': sample_id, 'random_select': self.random_select}
        sample_info2 = {'sample_id': sample_id+1, 'random_select': self.random_select}
        ...
        sample_info2['left_image'] = img_left2
        sample_info2['right_image'] = img_right2
        sample_info2['gt_depth'] = depth2
        sample_info2['calib'] = calib2
        sample_info2['gt_boxes3d'] = aug_gt_boxes3d

        return sample_info,sample_info2
