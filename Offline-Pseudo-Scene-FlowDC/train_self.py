...
def main():
    ...
    train_dataset = datasets.__dict__['KITTI'](
        train=True,
    ...
    val_dataset = datasets.__dict__['KITTIeval'](
        train=False,
    ...
    optimizer.param_groups[0]['initial_lr'] = args.learning_rate   # initial_lr
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=8, gamma=0.8, last_epoch = init_epoch - 1)
    ...
    #str_out = 'EPOCH %d %s mean epe3d: %f  mean eval loss: %f'%(0, blue('eval'), eval_epe3d, eval_loss)
    #print(str_out)
    print('========args.learning_rate ============:', optimizer.param_groups[0]['lr'])
    for epoch in range(init_epoch, args.epochs):
        ...
        total_depth_consistency = 0
        total_seen = 0
        optimizer.zero_grad()
        for i, data in tqdm(enumerate(train_loader, 0), total=len(train_loader), smoothing=0.9):
            pos1, pos2, norm1, norm2, flow, depth2, ii = data
            np.save('./points/Mono1_{}'.format(ii), pos1)
            np.save('./points/Mono2_{}'.format(ii), pos2)
            #move to cuda
            ...
            full_flow = pred_flows[0].permute(0, 2, 1).cpu().detach().numpy()
            np.save('./points/MonoFlow_{}'.format(ii), full_flow)
            loss, chamfer_loss, curvature_loss, smoothness_loss, depth_consistency_loss = multiScaleChamferSmoothCurvature(pc1, pc2, pred_flows, depth2)

            # history['loss'].append(loss.cpu().data.numpy())
            loss.backward()
            optimizer.step() 
            optimizer.zero_grad()
            print("Total Loss:",loss.cpu().data.numpy(), '\n', 'Chamfer Loss:', chamfer_loss.cpu().data.numpy(), '\n', "Curvature Loss", curvature_loss.cpu().data.numpy(), '\n', 'Smoothness Loss', smoothness_loss.cpu().data.numpy(), '\n', 'Depth Consistency Loss', depth_consistency_loss.cpu().data.numpy(),'\n')
            ...
            total_depth_consistency += depth_consistency_loss.cpu().data * args.batch_size
        ...
        depth_consistency = total_depth_consistency / total_seen
        str_out = 'EPOCH %d %s mean loss: %f'%(epoch, blue('train'), train_loss)
        print(str_out)
        logger.info(str_out)
        str_out4 = 'EPOCH %d %s mean depth consistency loss: %f'%(epoch, blue('train'), depth_consistency)
...
def eval_sceneflow(model, loader):
    ...
    for batch_id, data in tqdm(enumerate(loader), total=len(loader), smoothing=0.9):
        ...
        with torch.no_grad():
            ...
            eval_loss, chamfer_loss, curvature_loss, smoothness_loss, _ = multiScaleChamferSmoothCurvature(pc1, pc2, pred_flows, None)
