...
class Calibration(object):
    def __init__(self, calib_file):
        ...
        self.V2C = torch.from_numpy(calib['Tr_velo2cam']).cuda()  # 3 x 4
    ...
    def cart_to_hom(self, pts):
        ...
        """
        pts_hom = torch.cat([pts, torch.ones((pts.shape[0], pts.shape[1], 1), dtype = torch.float32).cuda()], dim=2)
        return pts_hom
    ...
    def lidar_to_rect(self, pts_lidar):
        ...
        pts_rect = torch.matmul(pts_lidar_hom, (torch.mm(self.V2C.T, self.R0.T)).expand(pts_lidar.shape[0],4,3))
        # pts_rect = reduce(torch.dot, (pts_lidar_hom, self.V2C.T, self.R0.T))
        return pts_rect
    ...
    def rect_to_img(self, pts_rect):
        ...
        #print('%*()^%^$%',pts_2d_hom[3, :, 0:2].permute(1,0)/pts_rect_hom[3, :, 2],'---------------',pts_2d_hom[0, :, 0:2].permute(1,0)/pts_rect_hom[0, :, 2], '=========')
        pts_img = (pts_2d_hom[:, :, 0:2].permute(0, 2, 1) / pts_rect_hom[:, :, 2].expand(2, pts_rect_hom.shape[0],pts_rect_hom.shape[1]).permute(1,0,2)).permute(0, 2, 1)  # (N, 2)
        # pts_img = pts_img.permute(0, 2, 1)
        #print('---------------',pts_img[0], pts_img[3],'+++++++++++++')
