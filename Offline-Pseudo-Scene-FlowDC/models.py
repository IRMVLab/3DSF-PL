...
def depth_consistency(depth_gt, pc_warp):
    # input BHW B3N
    HH, WW = depth_gt.shape[1], depth_gt.shape[2]
    pc_warp = pc_warp.permute(0, 2, 1)
    pc_warp_lidar = pc_warp[:,:,[2,0,1]]
    calib_file = '../../dataset/data_odometry_color/sequences/02/calib/000472.txt'
    assert os.path.exists(calib_file)
    calib = calibration.Calibration(calib_file)
    pts_img, warp_depth = calib.lidar_to_img(pc_warp_lidar) # pts_img --> BN2
    pts_img[:, :, 0] = pts_img[:, :, 0] / (WW - 1.0) * 2.0 - 1.0
    pts_img[:, :, 1] = pts_img[:, :, 1] / (HH - 1.0) * 2.0 - 1.0
    # pts_img[:, :, 0] = (pts_img[:, :, 0] / (WW - 1.0) - 0.5) * 2.0
    # pts_img[:, :, 1] = (pts_img[:, :, 1] / (HH - 1.0) - 0.5) * 2.0
    # input:  B 1 H W  ,  B 1 N 2   output: 
    depth_pc = torch.nn.functional.grid_sample(torch.unsqueeze(depth_gt,1), torch.unsqueeze(pts_img,1), padding_mode='border')
    depth_pc = depth_pc.squeeze(1).squeeze(1)
    DC_loss = (warp_depth - depth_pc).abs().mean()
    return DC_loss

def multiScaleChamferSmoothCurvature(pc1, pc2, pred_flows, depth2_loaded):
    f_curvature = 0.2
    f_smoothness = 0.3
    f_chamfer = 1.0
    f_DC = 1.0
    ...
    depth_consistency_loss = torch.zeros(1).cuda()
    for i in range(num_scale):
        cur_pc1 = pc1[i] # B 3 N
        cur_pc2 = pc2[i]
        ...
        # depth_consistency loss
        if depth2_loaded is not None:
            if i <2:
                DC_loss = depth_consistency(depth2_loaded, cur_pc1_warp)
            else:
                DC_loss = torch.zeros(1).cuda()
        else:
            DC_loss = torch.zeros(1).cuda()
        # print('-------',depth_consistency_loss, '====',DC_loss)
        
        chamfer_loss += alpha[i] * chamferLoss
        smoothness_loss += alpha[i] * smoothnessLoss
        curvature_loss += alpha[i] * curvatureLoss
        depth_consistency_loss += alpha[i] * DC_loss
    # if chamfer_loss>80:
    #     chamfer_loss = chamfer_loss * 0.3
    total_loss = f_chamfer * chamfer_loss + f_curvature * curvature_loss + f_smoothness * smoothness_loss + depth_consistency_loss * f_DC

    return total_loss, chamfer_loss, curvature_loss, smoothness_loss, depth_consistency_loss
