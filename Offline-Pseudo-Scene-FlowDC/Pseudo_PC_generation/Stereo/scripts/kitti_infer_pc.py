...
# if i == 620:
for i in range(0, 1):
    left = np.array(Image.open("/dataset/data_odometry_color/02/image_2/{:06d}.png".format(i)))
    right = np.array(Image.open("/dataset/data_odometry_color/02/image_3/{:06d}.png".format(i)))
    ...
    with torch.no_grad():
        ...
        disp_pred[occ_pred] = 1000000000.0
        
        calib = calibration.Calibration(calib_file)
        # pts_pc = calib.depthmap_to_lidar(disp_pred)
        depthmap = calib.disp_to_depth(disp_pred)

        #np.save('/dataset/data_odometry_color/02/Stereo_pc/{:06d}.npy'.format(i), pts_pc)
        np.save('/dataset/data_odometry_color/02/Stereo_depth/{:06d}.npy'.format(i), depthmap)
